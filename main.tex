...
\textbf{Survey Methodology:} To ensure rigorous coverage, our literature search targeted flagship conferences (e.g., CVPR, ICCV, ECCV, NeurIPS, ICLR) and journals (e.g., TPAMI, TOG, TIP) published between 2015 and 2025. Search keywords included ``portrait stylization'', ``style transfer'', ``GAN inversion'', and ``diffusion-based editing''. We excluded purely unconditional generation works, strictly including methods that offer identity-preserving or structure-preserving stylistic controls. This screening yielded over 115 peer-reviewed publications in our reference database. Of these, approximately 90 are distinct PST methods with dedicated stylization components; the remainder are foundational generative models (e.g., DDPM \cite{ho2020denoising}) included as theoretical prerequisites but not counted in the method taxonomy.
