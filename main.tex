...
In this survey, we identify a structural tension among three critical objectives. We term this the \textbf{"Portrait Editing Trilemma"}, an engineering trade-off framework used throughout the paper \cite{wang2024instantstyle,wang2025domaingp,yang_2022_cvpr,wright2022artfidqe}:
...
Existing models often sacrifice at least one of these strictly competing objectives. General-purpose editors (e.g., InstructPix2Pix~\cite{brooks2023instructpix2pix}) excel at fidelity but distort identity; specialized identity models (e.g., InstantID~\cite{wang2024instantid}) preserve likeness but often struggle with complex stylistic instructions or suffer from high latency. In diffusion pipelines, inference commonly uses tens of denoising steps; under typical training-free DDIM-family settings, stable stylization often falls in the 30--50 step range, while practical accelerated configurations are commonly reported around 20--50 steps \cite{song2020denoising,rombach_2022_cvpr}.

\subsection{Research Objectives and Contributions}
\label{sec:intro:contributions}
This paper makes the following principal contributions:
\begin{itemize}
  \item \textbf{Unified Mechanism Taxonomy (covering $\sim$12 NST, $\sim$25 GAN, $\sim$45 diffusion methods, inclusive of cited structural baselines within the discussed works, and $\sim$10 AR methods):} We provide a systematic categorization based on the underlying generative mechanisms rather than target applications, enabling cross-paradigm comparisons.
  \item \textbf{Critical Analysis of Failure Modes}: A taxonomy of "Killer Failures" including the Identity-Style Conflict and Texture Swimming.
...
\textbf{Survey Methodology:} To ensure rigorous coverage, our literature search targeted flagship conferences (e.g., CVPR, ICCV, ECCV, NeurIPS, ICLR) and journals (e.g., TPAMI, TOG, TIP) published between 2015 and 2025. Search keywords included ``portrait stylization'', ``style transfer'', ``GAN inversion'', and ``diffusion-based editing''. We excluded purely unconditional generation works, strictly including methods that offer identity-preserving or structure-preserving stylistic controls. Foundational generative models without stylization components (e.g., DDPM \cite{ho2020denoising}) are included as theoretical prerequisites but are not counted in the $\sim$90 distinct methods.
We further highlight recent one-shot and real-time portrait stylization systems as motivating use cases for the taxonomy and evaluation protocol \cite{lai2025instantportraitop,wang2024magicfacetu}. Figure~\ref{fig:teaser_trilemma} provides a visual teaser of the Trilemma framing, while Figure~\ref{fig:timeline_evolution} summarizes the mechanism-level timeline used in this survey.
...
\begin{figure*}[t]
  ...
  \caption{Portrait editing trilemma and representative result slots. Left: structural trade-offs among identity preservation, stylization intensity, and inference speed. Right: qualitative comparison slots to be filled with controlled method outputs.}
...
\begin{figure*}[t]
  ...
  \caption{Mechanism-centered timeline of portrait style transfer (2015--2025). The main branch illustrates the progression from optimization and feed-forward stylizers to GAN and diffusion paradigms, while newer AR/3D/video directions are treated as an emerging frontier branch.}
  \label{fig:timeline_evolution}
\end{figure*}

\input{Survey/paper/tabs/tab_chronology}
...
Loss functions act as ``navigational constraints'' that guide a model toward the desired balance of perceptual fidelity and artistic abstraction. Standard perceptual and style losses, derived from VGG-network feature maps or Gram matrices, align low-level textures. However, portraiture requires more robust semantic constraints to maintain identity, and perceptual similarity measures such as LPIPS are commonly used to quantify content deviations \cite{zhang2018lpips}.
...
\end{equation}
where the key difference from generic NST is the explicit identity and geometry terms. In practice, reported weighting strategies vary by architecture and objective normalization; common settings keep content/style terms at comparable scales in NST-style formulations \cite{gatys2016image,johnson2016perceptual,huang2017arbitraryst}, while portrait-identity pipelines usually increase identity constraints (often around $\lambda_{id}\approx0.5$--$1.0$) and apply lighter geometric regularization (often around $\lambda_{geo}\approx0.1$--$0.5$) \cite{yang2022dualstylegan,sain2023styleface}.
...
Modern generative models are deeply influenced by the non-parametric roots of image processing. Early methods, such as histogram matching and Laplacian pyramids, established the methodological foundations for makeup and color transfer by decomposing images into hierarchical components \cite{chen2018facess,luan2017deepps,li2018linearst,park2019arbitraryst}.
...
Diffusion models, operating through forward and reverse Markov chains, currently offer high diversity in portrait stylization \cite{ho2020denoising}. In addition to classic editing via stochastic differential equations, portrait-specific diffusion systems incorporate identity-aware constraints to avoid drift \cite{meng2021sdedit,kim2023diffface}. A key technical lever in this paradigm is \emph{attention control}: identity embeddings can be injected into cross-attention layers to stabilize identity while prompts guide texture and style \cite{wang2024instantid,zhang2023adding,ye2023ipadapter}.

Following identity-conditioned cross-attention formulations in adapter-based diffusion editors \cite{ye2023ipadapter,wang2024instantid}, one abstraction is to treat an identity embedding $e_c$ as key/value features for cross-attention:
\begin{equation}
  \mathrm{Attn}(Q_s^i, e_c, e_c) =
  \mathrm{softmax}\left(\frac{Q_s^i e_c^{T}}{\sqrt{d}}\right)e_c,
\end{equation}
where $Q_s^i$ is a query feature at layer $i$ and $d$ is the attention dimensionality. Following adapter alignment objectives \cite{zhang2023adding,wang2024instantid}, trainable modules can be constrained to remain close to content features:
\begin{equation}
  \mathcal{L}_{align} = \frac{1}{N} \sum_{i=1}^{N} \lVert Q_f^i - Q_c^i \rVert^2.
\end{equation}
where $Q_f^i$ denotes the adapted (fine-tuned) query feature at layer $i$, and $Q_c^i$ denotes the corresponding content-anchored query feature. In addition, following moment-matching style constraints in stylization models \cite{gatys2016image,huang2017arbitraryst}, statistic-consistency losses can encourage structure/style separation by matching moments across denoising predictions:
\begin{equation}
  \mathcal{L}_{style} =
...
Within the Adapter paradigm, the core architectures differ significantly: ControlNet utilizes spatial conditioning via residual feature injection \cite{zhang2023adding}, IP-Adapter employs decoupled cross-attention specifically for image prompts \cite{ye2023ipadapter}, and InstantID integrates ArcFace embeddings with weak spatial ControlNet conditioning \cite{wang2024instantid}. Quantitatively, these frameworks set new SOTA baselines; for instance, InstantStyle \cite{wang2024instantstyle} achieves an ArtFID of 42.48, significantly outperforming traditional tuning methods.

Quantitatively, this paradigm sets state-of-the-art benchmarks; recent evaluations demonstrate that methods like InstantStyle and StyleID achieve highly competitive ArtFID scores of 42.48 and 38.57 respectively, underscoring their dominance in balancing fidelity and texture \cite{ye2024stylemastersy}. To ensure global artistic coherence, recent systems also employ semantic-aligned adversarial objectives that operate on pretrained vision backbones (e.g., ViT feature manifolds \cite{kwon2022diffuseit}, a diffusion-based method discussed further in Section \ref{sec:methodology}, \cite{cheng2023visual}). Despite their strengths, diffusion models typically require explicit identity/structure controls to avoid identity drift and content leakage under strong stylization.

\section{Methods and Method Taxonomy}
\label{sec:methodology}

To avoid misleading cross-protocol ranking, we do not provide a single merged score table across heterogeneous test sets. Instead, quantitative evidence is discussed within each method family and task-specific subsection under their original experimental settings.

This section reviews how generative modeling frameworks have shaped portrait stylization. To provide a rigorous and symmetric analysis, we categorize the field into four foundational method families: NST \& Feature Transforms, Generative Adversarial Networks (GANs), Diffusion Models, and Autoregressive (AR) Models. Following the evaluation standards, each family is systematically analyzed through a unified framework: its core generative mechanism, representative methods with coverage statistics, inherent advantages, and fundamental limitations.
...
% ==========================================
% 3.1 NST and Feature Transforms
% ==========================================
\subsection{NST and Feature-Based Transforms}
...
\textbf{Representative Methods (Coverage: $\sim$12 papers):} The optimization-based paradigm was pioneered by Gatys et al. \cite{gatys2015ana}. To achieve real-time transfer, Huang et al. introduced AdaIN \cite{huang2017arbitraryst}, which became the standard for arbitrary style transfer. For portrait-specific applications requiring dense semantic correspondences, Luan et al. proposed Deep Photo Style Transfer \cite{luan2017deepps}, while subsequent works like Avatar-Net \cite{avatar-net} and AdaAttN \cite{liu2021adaattnra} integrated attention mechanisms to decorate semantic features locally without distorting facial layouts.

\textbf{Advantages:} These methods are highly efficient (in their feed-forward variants) and theoretically unbiased, capable of achieving zero-shot arbitrary style transfer without the need for extensive domain-specific pretraining. They mathematically guarantee the decoupling of structure and texture statistics.

\textbf{Limitations:} The primary failure mode is \textit{structural drift}. Since Gram matrices and global statistics are completely agnostic to spatial geometry, these methods often misalign facial components (e.g., applying eye makeup textures to the background). Furthermore, feed-forward transforms often blur high-frequency details, resulting in "washed-out" portrait textures. Quantitatively, classic NST approaches reflect these limitations; for instance, AdaIN yields an ArtFID of 31.85, highlighting a higher distributional gap compared to modern diffusion counterparts \cite{chung2023styleii}.
...
4) \textit{Inversion and editing:} DiffusionCLIP \cite{kim2022diffusionclip} remains a representative text-guided inversion/editing baseline in portrait-centric diffusion pipelines.
...
Video portrait stylization extends static style transfer to temporal sequences, where professional-grade results require both temporal coherence and identity stability \cite{ye2024stylemastersy,huang2023styleavideoad,yang2023rerender,liu2023portraitdt,yang2024fresco}. While per-frame stylization is straightforward, the central technical challenge is to prevent high-frequency temporal variance from producing perceptually disruptive artifacts.

The primary hurdle is \emph{flickering}: inconsistent stylization across consecutive frames that breaks the viewer's suspension of disbelief \cite{ye2024stylemastersy}. Furthermore, ReRender-A-Video \cite{yang2023rerender} proposes a zero-shot, text-guided video translation framework that enforces hierarchical cross-frame constraints to maintain global style coherence without requiring expensive per-video fine-tuning. Achieving temporal consistency is a non-trivial extension of both GAN and diffusion paradigms, since the model must accommodate motion while preserving identity-critical facial cues.
...
Stable-Makeup \cite{zhang2024stablemakeup} explicitly separates identity prompts from instruction prompts. These diffusion-based makeup systems combine CLIP-derived detail features with ControlNet-style structural constraints (a paradigm also shared by FreeUV \cite{li2024freeuv}) to preserve geometry.
...
3D-aware control mechanisms leverage parametric facial models and semantic directions (pose, expression, age) to preserve structural controllability during stylization. Such rigging-style controls provide stable geometry priors that help maintain identity under multi-view rendering \cite{zheng2024puffnet}.
...
To address the inconsistencies highlighted in prior works and improve cross-paper reproducibility, we formalize a comprehensive benchmark framework, termed the \textbf{"Golden Protocol"}. This protocol mandates a multi-pillar approach encompassing standardized experimental configurations, distribution-aware quantitative metrics, and rigorous human-centric qualitative studies.

% ==========================================
% 5.1 Standardized Configuration & Reproducibility
% ==========================================
\subsection{The Golden Protocol: Benchmark Configuration}
\label{sec:evaluation:configuration}

A persistent flaw in current literature is the "cherry-picking" of style-content pairs. To ensure a fair and reproducible evaluation, the Golden Protocol requires researchers to explicitly report the following standardized configurations:

\begin{itemize}
\item \textbf{Dataset Standardization:} Evaluations must be conducted on universally recognized test splits. We recommend sampling 1,000 to 5,000 paired combinations using high-quality content datasets (e.g., FFHQ or CelebA-HQ) and specific style domains (e.g., WikiArt for textures, Danbooru for anime, or AAHQ for artistic portraits). For prompt-based models, standard prompt suites from OmniStyle-1M (see Section~\ref{sec:datasets:omnistyle}) should be utilized to test long-tail style vocabularies.
\item \textbf{Sampling \& Hyperparameters:} For diffusion-based models, authors must strictly report the inference settings: the choice of sampler (e.g., DDIM), the exact number of denoising steps (typically 30--50 steps for training-free methods), and the Classifier-Free Guidance (CFG) scale.
\item \textbf{Reporting Format:} All quantitative results must be aggregated over fixed random seeds (e.g., 3-5 seeds per prompt/image) and reported in a strict \textit{Mean $\pm$ Standard Deviation} format. For deterministic models (e.g., GAN-based), seed variation applies to inversion optimization; report mean $\pm$ std over 3-5 inversion runs.
\end{itemize}
...
Following the configurations above, the generated outputs must be assessed along three distinct axes to reflect the Portrait Editing Trilemma:
...
\textbf{Human A/B Testing Protocol:} We standardize the user study methodology to a Two-Alternative Forced Choice (2AFC) or a Mean Opinion Score (MOS) framework \cite{ambiel2023portraitsa}. Evaluators must score outputs on a 5-point Likert scale across three isolated criteria: \textit{(i) Identity Preservation}, \textit{(ii) Style Faithfulness}, and \textit{(iii) Overall Aesthetic Preference}. To ensure statistical validity, studies should include a minimum of 20 distinct style-content pairs assessed by at least 25 independent raters, with reported inter-rater agreement (e.g., Fleiss' Kappa). To reduce demographic bias in aesthetic judgments, rater pools should include diverse age, gender, and cultural backgrounds.
...
\textbf{Large Multimodal Models (LMMs) as Evaluators:} Driven by the high cost of human crowdsourcing, recent pipelines incorporate advanced LMMs (e.g., GPT-4o \cite{hurst2024gpt4o}) as scalable proxies for qualitative scoring. Under the Golden Protocol, when using LMMs, the evaluation prompt must be explicitly structured to mimic human cognitive processes via Chain-of-Thought (CoT) reasoning. The prompt should explicitly instruct the LMM to disentangle "biometric preservation" from "stylistic abstraction". Preliminary observations suggest LMMs may exhibit biases towards color saturation and fail to detect subtle high-frequency artifacts (e.g., texture swimming in video); thus, they should augment rather than entirely replace human-centric studies.
...
\midrule
FFHQ \cite{karras2018progressive} & 2019 & 70k & Real Faces & CC BY-NC-SA 4.0 (Research) & High-resolution face prior for GAN inversion. \\
CelebAMask-HQ \cite{lee2020} & 2020 & 30k & Real+Labels & Non-Commercial; redistribution-sensitive & Semantic parsing for region-aware local editing. \\
Danbooru \cite{danbooru2021} & 2021 & 4.9M & Anime Art & BSD software layer; artwork rights mixed/high-risk & Tag-based controllable anime stylization. \\
WikiArt \cite{wikiart} & 2015 & 250k+ & Artworks & Public Domain (Mixed) & General artistic textures and brushstroke baseline. \\
AAHQ & 2021 & 25k & Artistic Faces & Research-Only & Human-centric artistic portrait benchmark. \\
OmniStyle-1M \cite{wang2025omnistyle} & 2025 & 1M & Synthetic Triplets & Open for Research & Text-image triplets for long-tail instruction editing. \\
CelebV-HQ \cite{celebvhq} & 2022 & 35k clips & Real Video & Non-Commercial (Research) & Temporal consistency benchmarks for video. \\
StyleGAN-Human \cite{stylegan-human} & 2022 & 40k+ & Synthetic Faces & Open (Privacy-Compliant) & Diversity-rich pretraining without real-face GDPR risk. \\
\bottomrule
\end{tabular}
\end{table*}
...
Following the metric definitions in Section~\ref{sec:evaluation:configuration}, this section focuses on practical interpretation rather than redefining each metric. In specialized portraiture, standard computer vision metrics designed for generic reconstruction can be misaligned with stylization objectives. A successful stylization must navigate a high-dimensional manifold in which identity-preserving structure coexists with transformative aesthetics. As a result, quantitative analysis should jointly examine (i) identity integrity via face-embedding similarity, (ii) style alignment via multimodal encoders, and (iii) perceptual distribution matching using generative quality metrics.

\subsubsection{Identity Preservation and Facial Integrity}
\label{sec:evaluation:metrics:identity}
Identity preservation is a primary benchmark in portrait evaluation. Modern frameworks employ pretrained face recognition networks (e.g., ArcFace/InsightFace/FaceNet-like encoders) to compute identity similarity as cosine similarity in a hyperspherical embedding space \cite{deng2019arcface}. Compared with pixel-level or landmark-only constraints, embedding-based scores reflect higher-level biometric cues and are therefore more aligned with human perception of ``who'' a person is.

These encoders, however, have limitations: most are trained on photorealistic face datasets and may be biased toward photorealistic features. Under highly abstract target styles (e.g., minimalist sketches), identity embeddings can become unreliable because biometric cues are intentionally suppressed. This motivates complementary structure-preserving measures (e.g., edge/landmark constraints or Canny-guided controllers) that evaluate spatial geometry rather than identity in a latent recognition space.

\subsubsection{Style Alignment and Text--Image Consistency}
\label{sec:evaluation:metrics:style}
Evaluation has shifted from purely reference-based metrics toward reference-free paradigms enabled by vision--language models. CLIP-like similarities allow alignment to be evaluated against text prompts (or style descriptors) without requiring paired ground truth stylizations. In practice, style alignment is often quantified using rescaled cosine similarity between textual embeddings and visual embeddings; reference-augmented variants can combine prompt alignment with similarity to a style reference.

Reference-free metrics provide practical advantages: they scale to large datasets without paired supervision, generalize across visual domains (e.g., paintings and illustrations), and often correlate better with human judgments than n-gram captioning metrics \cite{gal2022image,zhang2023ssrencoderes}. However, CLIP-based scores can still be sensitive to prompt templates and should be interpreted alongside other measures.

\subsubsection{Perceptual Quality and Distribution Matching}
\label{sec:evaluation:metrics:quality}
Holistic quality assessment commonly uses FID and perceptual similarity measures (e.g., LPIPS-like metrics) \cite{heusel2017ttur,zhang2018lpips}. In stylization, FID is sometimes decomposed into a content-oriented distance and a style-oriented distance to isolate model behaviors. Importantly, ``content distance'' metrics can be paradoxical for portraits: strong stylization intentionally modifies color/texture and sometimes geometry; therefore, a near-perfect content-preservation score may indicate under-stylization rather than success. Robust evaluation should therefore be interpreted in light of the target style regime.

\paragraph{Metric applicability and paradoxes.}
To avoid metric misuse, we recommend reporting applicability conditions together with each score. Identity embedding metrics are most reliable under photorealistic and mildly stylized regimes, while abstract or geometry-altering styles require stronger reliance on human studies and structure-based checks. Prompt-alignment metrics are appropriate for text-driven settings but should be template-controlled and variance-reported across prompt paraphrases. For decomposed ``content/style'' distances, the interpretation must be regime-aware: in deformation-heavy styles (e.g., anime-like eye/face-shape shifts), improved ``content distance'' can indicate weaker stylization rather than better quality, so conclusions should be made jointly with style-strength and human preference results.

\subsection{Qualitative Evaluation and Human Studies}
\label{sec:evaluation:human}
User studies remain essential because stylization quality is ultimately determined by whether outputs are both recognizable and aesthetically coherent to human observers. Human-centric evaluation is particularly important for diagnosing failures that distributional metrics may miss, and recent portrait stylization studies formalize these protocols to align with ArtFID-like assessments \cite{wright2022artfidqe,ambiel2023portraitsa}.

\subsubsection{User Study Methodologies and Preference Mapping}
\label{sec:evaluation:human:userstudy}
A practical protocol is to score outputs along three axes: identity preservation, style transfer strength/faithfulness, and overall preference. A standard 5-point Likert scale can be used to reduce variance across raters, and results can be reported with confidence intervals and inter-rater agreement statistics when applicable \cite{wright2022artfidqe,ambiel2023portraitsa}.

Recently, large multimodal language models have been explored as scalable proxies for preference scoring, by prompting the model to rate identity and style consistency. Such proxies can improve throughput but should be treated as complementary to human studies, not replacements.

\subsubsection{Visual Artifact Analysis and Failure Modes}
\label{sec:evaluation:human:artifacts}
Qualitative analysis often categorizes failure modes into (i) content leakage, where unintended elements from a style reference appear in the portrait, and (ii) identity distortion, where geometric changes to facial components break recognizability. A recurring trap is high-strength stylization: increasing style strength parameters can cause identity collapse into a generic face in the target domain. Understanding and mitigating this trade-off motivates the architectural innovations discussed next.
