...
Following identity-conditioned cross-attention formulations in adapter-based diffusion editors \cite{ye2023ipadapter,wang2024instantid}, one abstraction is to treat an identity embedding $e_c \in \mathbb{R}^{d_k}$ as key/value features for cross-attention:
\begin{equation}
  \mathrm{Attn}(Q_s^i, e_c, e_c) =
  \mathrm{softmax}\left(\frac{Q_s^i e_c^{T}}{\sqrt{d_k}}\right)e_c,
\end{equation}
where $Q_s^i \in \mathbb{R}^{d_k}$ is a query feature at layer $i$ and $d_k$ is the key dimension. Following adapter alignment objectives \cite{zhang2023adding,wang2024instantid}, trainable modules can be constrained to remain close to content features using adapted query features $Q_f^i$ and content-anchored queries $Q_c^i$:
\begin{equation}
