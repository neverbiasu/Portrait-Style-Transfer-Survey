...
\noindent\textbf{Definition 1 (Portrait Style Transfer).} Given a source portrait $\mathbf{x}$ and a style reference $\mathbf{s}$ (an image, a text prompt, or a learned embedding), \emph{Portrait Style Transfer (PST)} is the task of synthesizing a portrait $\mathbf{y} = G(\mathbf{x}, \mathbf{s}; \theta)$ with a generative model $G$ parameterized by $\theta$, such that (i)~the subject's identity and facial topology are preserved, (ii)~the target visual style is faithfully conveyed, and (iii)~$\mathbf{y}$ remains a plausible, controllable portrait. Operationally this minimizes a joint objective
$\mathcal{L}_{\mathrm{PST}} = \mathcal{L}_{\mathrm{style}}(\mathbf{y},\mathbf{s}) + \lambda_{\mathrm{id}}\,\mathcal{L}_{\mathrm{id}}(\mathbf{y},\mathbf{x}) + \lambda_{\mathrm{ctrl}}\,\mathcal{L}_{\mathrm{ctrl}}(\mathbf{y},\mathbf{x})$,
subject to an identity-preservation constraint $I(\mathbf{y},\mathbf{x})\ge \tau_{\mathrm{id}}$ and a practical compute budget.\hfill$\diamond$
...
\begin{enumerate}
  ...
  \item \textbf{Stylization Strength}: The model must convey the target visual style with sufficient artistic intensity while keeping the result legible as a portrait.
  \item \textbf{Computational Efficiency}: The model must produce edits under a practical latency and compute budget (ideally in real time), to remain viable for interactive deployment.
\end{enumerate}

We adopt the well-established \emph{impossible triangle} of stylization---the competing tension among identity preservation, stylization strength, and computational efficiency---as the primary analytical lens of this survey, formalizing it as the \textbf{Portrait Editing Trilemma} that organizes the three axes above.
...
Formally, let a method $m$ be characterized by its identity-preservation score $I_m$, stylization-strength score $\mathrm{St}_m$, and computational-efficiency score $C_m$ (higher is better on all axes). $m$ \emph{Pareto-dominates} $m'$ if $I_m \ge I_{m'}$, $\mathrm{St}_m \ge \mathrm{St}_{m'}$, and $C_m \ge C_{m'}$ with at least one strict inequality. The Pareto frontier $\mathcal{P}$ is the set of non-dominated methods. Qualitatively: NST methods achieve high stylization strength and moderate speed but weak identity; GANs offer strong identity and fast speed yet limited stylization range; diffusion models attain high identity and stylization strength at the cost of efficiency; AR models balance structure control and efficiency but introduce quantization artifacts. This mapping is formalized in the unifying framework (Section~\ref{sec:theory:unified}).
...
\textbf{Survey Methodology:} To ensure rigorous coverage, we conducted a systematic literature search following PRISMA-inspired guidelines. Our search targeted flagship venues---CVPR, ICCV, ECCV, NeurIPS, ICLR, AAAI, ACM MM, SIGGRAPH (conferences) and TPAMI, TOG, IJCV, TVCG, TIP (journals)---published between 2015 and 2025. Keywords comprised four query categories: ``portrait stylization'', ``style transfer'', ``GAN inversion'', and ``diffusion-based editing''. The initial search returned approximately 480 records. After removing duplicates, approximately 320 unique records proceeded to title and abstract screening, which excluded purely unconditional generation, non-face stylization, and methods without explicit identity-preserving or structure-preserving controls, narrowing the pool to approximately 180 candidates. Full-text eligibility assessment yielded a final corpus of 115+ publications, comprising 88 peer-reviewed venues and 28 screened arXiv preprints. Of these, approximately 90 are distinct PST methods with dedicated stylization components; the remainder are foundational generative models (e.g., DDPM \cite{ho2020denoising}) included as theoretical prerequisites. The venue distribution spans CVPR (28), SIGGRAPH/TOG (10), ICCV (8), ECCV (5), NeurIPS and ICLR (12), TPAMI and other journals (10), and additional conferences (15), summing to 88 peer-reviewed venues, with a further 28 arXiv preprints cited as emerging contributions. Relative to existing surveys---Kyprianidis et al.'s NPR taxonomy \cite{kyprianidis2013state}, Jing et al.'s NST survey \cite{jing2020neural}, Xia et al.'s GAN inversion survey \cite{xia2023gan}, the general style-transfer decade surveys of Zhang and Tang \cite{zhang2025decadesurvey} and Xu et al. \cite{xu2025stylereview}, and the personalized image-generation decade survey of Zhang et al. \cite{zhang2025personalized}---our work offers the first portrait-focused synthesis that bridges pre-deep-learning NPR foundations with modern generative paradigms and provides a unified evaluation protocol. Concretely, relative to NPR- and NST-centric reviews \cite{kyprianidis2013state,jing2020neural}, we restrict scope to human portraits and treat identity preservation as the central constraint rather than one metric among many; relative to inversion-focused surveys \cite{xia2023gan}, we extend coverage through 2024--2025 generative paradigms (diffusion, AR, video, and 3D/NeRF frontiers) and contribute the Portrait Editing Trilemma as a unifying analytical framework together with the proposed Golden Protocol for standardized benchmarking.
...
\begin{figure*}[t]
  ...
  \caption{Portrait editing trilemma concept. Left: structural trade-offs among identity preservation, stylization strength, and computational efficiency. Right: conceptual illustration of the Trilemma axes that frame the paradigm analysis in Section~\ref{sec:methodology}.}
  \label{fig:teaser_trilemma}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{images/timeline.png}
  \caption{Timeline of key milestones in Portrait Style Transfer (2015--2025), organized by generative paradigm and colored by the dominant methodological family. The trajectory moves from heuristic texture-matching (NST) through GAN-based geometric manipulation to diffusion-driven open-domain stylization, with emerging autoregressive, video, and 3D/NeRF frontiers---mirroring the paradigm shift analyzed under the Portrait Editing Trilemma.}
  \label{fig:pst_timeline}
\end{figure*}
...
\end{equation}
This constraint has become a de facto standard in portrait stylization and identity-preserving editing, especially when the target style includes strong texture changes or partial geometric deformation. The inverse problem---recovering the original identity from an already-stylized portrait---has likewise been studied as an identity-preserving recovery task \cite{shiri2019identitypreservingfacerecoverystylized}.
...
\begin{table*}[t]
\centering
\caption{Unified tripartite decomposition of PST paradigms. Each row maps the same abstract components---control signal $c$, generative prior $p_\theta$, and constraint $\mathcal{L}$---to concrete instantiations across the four method families, with representative methods cited as examples.}
\label{tab:unified_paradigms}
\small
\begin{tabularx}{\textwidth}{@{}p{2.5cm}XXXp{4.3cm}@{}}
\toprule
\textbf{Paradigm} & \textbf{Control} $(c)$ & \textbf{Prior} $(p_\theta)$ & \textbf{Constraint} $(\mathcal{L})$ & \textbf{Representative methods} \\
\midrule
NST / Optimization \& Feed-forward & Style reference image & VGG feature hierarchy & Gram-matrix matching + content loss & Gatys NST \cite{gatys2015ana}, AdaIN \cite{huang2017arbitraryst}, Johnson FST \cite{johnson2016perceptual} \\
GAN-based & Inverted latent $w^+$ & StyleGAN face manifold & Adversarial + identity + reconstruction & StyleGAN \cite{karras2019style}, BlendGAN \cite{liu2021blendgan}, DualStyleGAN \cite{yang2022dualstylegan} \\
Diffusion & Text / style / image embedding & Score function $\nabla_x \log p_t$ & Denoising + CFG + identity & LDM/SD \cite{rombach_2022_cvpr}, IP-Adapter \cite{ye2023ipadapter}, InstantID \cite{wang2024instantid} \\
Autoregressive & Discrete visual / text tokens & Transformer next-token distribution & Cross-entropy over codebook + semantics & StyleTokenizer \cite{li2024styletokenizerdi}, EditAR \cite{mu2025editar}, VAR \cite{tian2024var} \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{table*}[t]
\centering
\caption{Qualitative mapping of PST paradigms to the three Portrait Editing Trilemma axes---identity preservation, stylization strength, and computational efficiency. $\bigstar$ = strong, $\blacktriangle$ = moderate, $\bullet$ = weak.}
\label{tab:trilemma_paradigms}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Paradigm} & \textbf{Identity Preservation} & \textbf{Stylization Strength} & \textbf{Computational Efficiency} \\
\midrule
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
      hub/.style={draw, rounded corners=3pt, fill=blue!8, text width=3.7cm, align=center, font=\footnotesize\bfseries},
      leaf/.style={draw, rounded corners=2pt, fill=gray!7, text width=3.1cm, align=center, font=\scriptsize},
      root/.style={draw, rounded corners=4pt, fill=blue!20, text width=4.4cm, align=center, font=\small\bfseries},
      >=Stealth, line width=0.6pt
    ]
    \node[root] (R) at (0,0) {Portrait Style Transfer (PST)};
    \node[hub] (H1) at (-7.6,-2.2) {Axis I:\\Generative Paradigm};
    \node[hub] (H2) at (-2.5,-2.2) {Axis II:\\Task / Output};
    \node[hub] (H3) at (2.5,-2.2) {Axis III:\\Control Signal};
    \node[hub] (H4) at (7.6,-2.2) {Analytical Lens:\\Trilemma};
    \draw (R) -- (H1); \draw (R) -- (H2); \draw (R) -- (H3); \draw (R) -- (H4);
    \node[leaf] (a1) at (-7.6,-4.0) {NST / Optimization \& Feed-forward};
    \node[leaf] (a2) at (-7.6,-5.3) {GAN-based (Geometric Manipulation)};
    \node[leaf] (a3) at (-7.6,-6.6) {Diffusion (Open-Domain Texture)};
    \node[leaf] (a4) at (-7.6,-7.9) {Autoregressive (Tokens \& Structure)};
    \node[leaf] (a5) at (-7.6,-9.2) {Flow / Hybrid};
    \foreach \x in {a1,a2,a3,a4,a5} \draw (H1) -- (\x);
    \node[leaf] (b1) at (-2.5,-4.0) {Single-image Stylization};
    \node[leaf] (b2) at (-2.5,-5.3) {Video (Temporal Consistency)};
    \node[leaf] (b3) at (-2.5,-6.6) {Local / Makeup Transfer};
    \node[leaf] (b4) at (-2.5,-7.9) {3D / NeRF-Aware};
    \foreach \x in {b1,b2,b3,b4} \draw (H2) -- (\x);
    \node[leaf] (c1) at (2.5,-4.0) {Pixel (loss-based)};
    \node[leaf] (c2) at (2.5,-5.3) {Latent code (inversion)};
    \node[leaf] (c3) at (2.5,-6.6) {Embedding (adapter / IP)};
    \node[leaf] (c4) at (2.5,-7.9) {Discrete token (AR)};
    \foreach \x in {c1,c2,c3,c4} \draw (H3) -- (\x);
    \node[leaf] (d1) at (7.6,-4.0) {Identity Preservation};
    \node[leaf] (d2) at (7.6,-5.3) {Stylization Strength};
    \node[leaf] (d3) at (7.6,-6.6) {Computational Efficiency};
    \foreach \x in {d1,d2,d3} \draw (H4) -- (\x);
  \end{tikzpicture}}
  \caption{Taxonomy of Portrait Style Transfer. The field decomposes along three orthogonal axes---generative paradigm (Axis I), task/output (Axis II), and control signal (Axis III)---each instantiated by the method families reviewed in this survey; the Portrait Editing Trilemma (identity preservation, stylization strength, computational efficiency) serves as the unifying analytical lens across all branches.}
  \label{fig:pst_taxonomy}
\end{figure*}
...
\begin{table*}[t]
\centering
\caption{Capability matrix of representative PST methods. Qualitative symbols: $\bigstar$ = strong/fast; $\blacktriangle$ = moderate; $\bullet$ = weak/slow. Speed figures are reported heterogeneously across the source papers (varying GPUs, resolutions, batch sizes, and backbone sizes) and are therefore \textit{not} directly comparable; they indicate relative orders of magnitude only, under each method's original evaluation protocol. The $\bigstar$/$\blacktriangle$/$\bullet$ ratings are relative qualitative judgments derived from each method's original publication, not measurements produced by this survey.}
...
\begin{table*}[t]
\centering
\caption{Representative portrait style-transfer methods surveyed in this work, organized by generative paradigm and advanced task axis. Contributions are stated from each method's original publication; this table complements Table~\ref{tab:comparison_matrix} (qualitative capability ratings) with an objective, citation-grounded overview.}
\label{tab:method_overview}
\small
\begin{tabular}{llp{9.0cm}}
\toprule
\textbf{Method} & \textbf{Paradigm} & \textbf{Core Contribution} \\
\midrule
Gatys et al. \cite{gatys2015ana} & NST & First CNN Gram-matrix neural style transfer. \\
AdaIN \cite{huang2017arbitraryst} & NST & Real-time arbitrary style transfer via adaptive instance normalization. \\
Deep Photo \cite{luan2017deepps} & NST & Photorealistic stylization with semantic correspondence. \\
Avatar-Net \cite{avatar-net} & NST & Multi-scale feature decoration for local style transfer. \\
AdaAttN \cite{liu2021adaattnra} & NST & Attention-aware adaptive normalization. \\
ArtFlow \cite{artflow} & NST & Invertible bijective flow for unbiased transfer. \\
Puff-Net \cite{zheng2024puffnet} & NST & Efficient 2D NST via content--style feature fusion. \\
\midrule
DualStyleGAN \cite{yang2022dualstylegan} & GAN & Intrinsic/extrinsic disentangled StyleGAN stylization. \\
pSp/e4e \cite{richardson2021encoding} & GAN & Encoder-based GAN inversion in the $W^+$ space. \\
AgileGAN \cite{song2021agilegan} & GAN & Hierarchical VAE inversion for few-shot stylization. \\
StyleGAN-NADA \cite{gal2021stylegannada} & GAN & CLIP-guided domain adaptation of generators. \\
\midrule
InstantID \cite{wang2024instantid} & Diffusion & ArcFace cross-attention for identity control. \\
IP-Adapter \cite{ye2023ipadapter} & Diffusion & Decoupled cross-attention adapter. \\
DreamBooth \cite{ruiz2023dreambooth} & Diffusion & Few-shot subject token tuning. \\
PhotoMaker \cite{li2024photomaker} & Diffusion & Stacked ID-embedding personalization. \\
PULID \cite{guo2024pulid} & Diffusion & Contrastive identity alignment. \\
ConsistentID \cite{huang2024consistentid} & Diffusion & Multimodal fine-grained identity control. \\
StyleShot \cite{gao2024styleshot} & Diffusion & Style-aware encoder, tuning-free stylization. \\
InstantStyle \cite{wang2025instantstyleplus} & Diffusion & Reference-free style injection. \\
StyleMaster \cite{ye2024stylemastersy} & Diffusion & Video stylization via motion adaptation. \\
FlowEdit/DVRF \cite{flowedit2025,dvrf2025} & Diffusion & Rectified-flow ODE editing. \\
\midrule
EditAR \cite{mu2025editar} & AR & Feature distillation with token editing. \\
VAR \cite{tian2024var} & AR & Next-scale autoregressive prediction. \\
MaskGIT \cite{chang2022maskgit} & AR & Bidirectional masked generative modeling. \\
\midrule
AnimateDiff \cite{guo2024animatediff} & Video & Latent motion module for animation. \\
MagicAnimate \cite{xu2024magicanimate} & Video & Pose-guided human video animation. \\
AnimateAnyone \cite{hu2024animateanyone} & Video & Pose-driven character animation. \\
LivePortrait \cite{guo2024liveportrait} & Video & Implicit keypoint portrait animation. \\
\midrule
GaussianHair \cite{luo2024gaussianhair} & 3D & 3D-aware hair/gaussian stylization. \\
3DGS-Avatar \cite{qian20243dgsavatar} & 3D & 3D Gaussian Splatting avatar. \\
\midrule
Barbershop \cite{zhu2021barbershop} & Local & Localized editing via latent composition. \\
\bottomrule
\end{tabular}
\end{table*}
...
\label{sec:advanced}
Beyond the core paradigms above, portrait stylization is further organized along a primary axis of \textit{control granularity}---global versus local editing---which cuts across all generative families and determines whether identity-preserving structure is modified holistically or only within semantically parsed regions (e.g., makeup or attribute transfer). This granularity axis compounds with two further dimensions of increasing complexity: from static images to temporally coherent video, and from 2D to 3D-aware multiview generation. This section surveys these advanced tasks, organized by the spatial (local/global), temporal, and geometric dimensions they address.

\subsection{Video Portrait Stylization (Temporal Consistency)}
\label{sec:advanced:video}
Video portrait stylization extends static style transfer to temporal sequences, where professional-grade results require both temporal coherence and identity stability \cite{ruder2018artistic,ye2024stylemastersy,huang2023styleavideoad,yang2023rerender,liu2023portraitdt,yang2024fresco}. While per-frame stylization is straightforward, the central technical challenge is to prevent high-frequency temporal variance from producing perceptually disruptive artifacts. Content-preserving formulations that explicitly minimize structural deviation have also been proposed for both images and videos \cite{zhang2026telestylecontentpreservingstyletransfer}.
...
\label{sec:evaluation:benchmark_scope}
We clarify the scope of benchmarking in this survey. The literature screening that grounds our corpus is reported transparently as a PRISMA-style funnel (Table~\ref{tab:prisma}), and cross-paradigm comparability is provided qualitatively through the capability matrix (Table~\ref{tab:comparison_matrix}) and the metric-reporting frequency analysis (Table~\ref{tab:metric_usage}). We deliberately \textit{do not} present an executed numerical leaderboard that ranks methods on a single benchmark: doing so honestly would require re-implementing and running every surveyed method under one identical protocol, datasets, and hardware---a controlled experimental study beyond the scope of a literature survey, and one that current source papers cannot support because they report heterogeneous setups (see the speed caveat in Table~\ref{tab:comparison_matrix}). Instead, the proposed Golden Protocol (Section~\ref{sec:evaluation:configuration}) defines the standardized configuration under which such a leaderboard \textit{should} be constructed; we position its full empirical execution across all surveyed methods as essential future work and a call for community adoption, rather than a claim already fulfilled here. To demonstrate the protocol's concrete value, however, we execute a \textit{representative pilot} of it below.

\subsection{A Visual Gallery under the Golden Protocol}
\label{sec:evaluation:gallery}

To make the benchmarking discussion tangible and to expose the Portrait Editing Trilemma across paradigms, we execute a pilot of the Golden Protocol on a \textit{unified} content portrait and a \textit{unified} set of six public-domain style references (Van Gogh's \emph{Starry Night}, Munch's \emph{The Scream}, Hokusai's \emph{Great Wave}, Wood's \emph{American Gothic}, Monet's \emph{Water Lilies}, and Picasso's \emph{Les Demoiselles d'Avignon}). Every method stylizes the \emph{same} content and receives the \emph{same} style set, so differences are attributable to the method rather than to cherry-picked pairs. We deliberately restrict the pilot to three representative methods spanning the survey's paradigms---Gatys \cite{gatys2016image} (optimization-based), AdaIN \cite{huang2017arbitraryst} (feed-forward encoder), and IP-Adapter \cite{ye2023ipadapter} (diffusion-based)---rather than re-implementing all $\sim$90 surveyed methods; the pilot is illustrative, not an exhaustive leaderboard.

Figure~\ref{fig:gallery_overview} highlights the protocol in one strip: the unified content alongside one output per paradigm on \emph{Starry Night}, making the identity/stylization/structure trade-off immediately visible. Figure~\ref{fig:gallery_grid} expands this into a methods-$\times$-styles matrix, where the content occupies the first column and each row is a paradigm; the matrix lets a reader compare, for instance, how optimization-based and diffusion-based methods each cope with the high-frequency geometry of \emph{Great Wave} versus the flat planes of \emph{Cubism}. Finally, Figure~\ref{fig:gallery_failures} isolates a canonical failure mode: the optimization-based method's structural drift on a geometrically aggressive style (left) versus the spatial control afforded by the diffusion-based adapter (right), directly motivating the \emph{structural-drift} and \emph{content-leakage} caveats discussed in Sections~\ref{sec:methodology:nst}--\ref{sec:methodology:diffusion}.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.92\textwidth]{images/gallery_overview.png}
  \caption{Pilot Golden-Protocol gallery (paradigm highlight): one unified content portrait stylized by a representative method from each paradigm on Van Gogh's \emph{Starry Night}. Differences reflect the method, not the input pair.}
  \label{fig:gallery_overview}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{images/gallery_grid.png}
  \caption{Methods-$\times$-styles matrix under the unified Golden-Protocol inputs. The first column is the shared content portrait; each row is a paradigm (optimization-based, feed-forward encoder, diffusion-based). The matrix exposes how each paradigm trades off identity preservation, stylization strength, and structural fidelity across six public-domain styles.}
  \label{fig:gallery_grid}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.62\textwidth]{images/gallery_failures.png}
  \caption{Failure-mode comparison on a geometrically aggressive style (Picasso's \emph{Les Demoiselles d'Avignon}): optimization-based structural drift (left) versus the spatial control of the diffusion-based adapter (right), motivating the structural-drift and content-leakage caveats in the paradigm analyses.}
  \label{fig:gallery_failures}
\end{figure*}

The pilot is reproducible: the engine (\texttt{gallery/gallery.py}) and its Colab notebook (\texttt{gallery/PST\_Gallery.ipynb}) download the unified assets, run each method in isolation (so a single failure yields a smaller grid rather than a crash), and emit the three figures above. We stress that this gallery is a \textit{demonstration} of the Golden Protocol, not a claim of comprehensive benchmarking; the full multi-pillar quantitative study (Section~\ref{sec:evaluation:quantitative}) remains a call for community adoption.
...
The deployment of portrait stylization is increasingly governed by concrete legal frameworks rather than generic ``responsible AI'' principles, with two obligations most salient for PST. First, transparency mandates such as the EU AI Act (Regulation (EU) 2024/1689) \cite{euaiact2024} require explicit watermarking or disclosure of AI-generated synthetic media to curb misuse. Second, privacy regimes such as GDPR treat identifiable facial embeddings as sensitive biometric data, imposing data-minimization, lawful-basis, and right-to-erasure workflows \cite{gdpr2016}; for subject-driven personalization, this extends to lifecycle management of adapter checkpoints and cached identity embeddings, which preliminary analyses suggest may encode user-specific traits \cite{carlini2023extracting, shamshad2023clip2protect}. These constraints are operationalized in our dataset and privacy discussion (Section~\ref{sec:datasets}) and motivate the documentation and disclosure requirements of the proposed Golden Protocol (Section~\ref{sec:evaluation}).
...
This work is a literature survey and introduces no new datasets, models, or executable code. All methods, metrics, and datasets discussed are cited to their primary sources in the reference list. To support reproducibility of the survey itself, the curated BibTeX of the 115+ surveyed references and the qualitative capability matrix (Table~\ref{tab:comparison_matrix}) are maintained by the authors and available upon reasonable request.

\subsection{Practical Guidelines: Lessons from a Decade of PST}
\label{sec:discussion:guidelines}

Beyond taxonomy and benchmarking, a decade of portrait stylization research yields actionable, experience-based guidance. We consolidate the most consequential lessons as practical guidelines for practitioners and future benchmark designers.

\begin{itemize}
  \item \textbf{G1 -- Choose the paradigm by your binding constraint, not by trend.} The generative family should be selected from the dominant limiting axis of the Trilemma (Fig.~\ref{fig:pst_taxonomy}): when identity fidelity is non-negotiable and the domain is fixed, GAN priors still win; when open-vocabulary style range matters more than latency, diffusion is the default; when structural/discrete control is the goal, autoregressive paradigms are emerging as the natural fit.
  \item \textbf{G2 -- Treat identity preservation as a hard constraint, not a late metric.} Retrofitting identity after stylization almost always trades away style. Enforce it at the representation level (face-embedding losses, inversion-based conditioning, or adapter-gated identity paths) so that stylization operates within the identity manifold rather than correcting for its violation.
  \item \textbf{G3 -- Disentangle style from content at the representation, not the pixel, level.} Pixel-space losses (e.g., naive Gram/IN statistics) couple style and structure and produce the artifacts the field has spent years fighting. Latent feature modulation, decoupled reference encoders, and Q-Former-style extraction are the more robust modern recipe.
  \item \textbf{G4 -- Standardize evaluation \emph{before} claiming SOTA.} Cross-method comparison is only meaningful under a shared protocol; the proposed \emph{Golden Protocol} (Section~\ref{sec:benchmark}) exists precisely because ad-hoc settings inflate gains. Report identity, style, and efficiency jointly, never a single axis in isolation.
  \item \textbf{G5 -- Respect the privacy shift in training data.} Scraped face datasets carry licensing and copyright risk; prefer large-scale synthetic triplets (e.g., OmniStyle-1M) and \emph{always} complement them with real-face evaluation (FFHQ) to surface generative bias.
  \item \textbf{G6 -- Latency is a first-class requirement, not a footnote.} Interactive deployment binds you to accelerated samplers and lightweight adapters; match the sampler/adapter budget to the target device rather than reporting best-case offline quality.
  \item \textbf{G7 -- For video and 3D, consistency dominates style.} In temporal and geometric domains, flicker and topology drift are perceived before stylistic nuance; prioritize cross-frame/geometric consistency mechanisms over raw texture fidelity.
\end{itemize}
...
Despite expanded coverage, this survey still has several limitations. First, quantitative reporting remains heterogeneous across datasets, protocols, and metrics, which limits direct cross-paper comparability. This is why the Golden Protocol (Section \ref{sec:evaluation}) focuses on future standardization rather than retrospective harmonization. Second, several emerging tracks (especially fast-moving 3DGS and flow-based preprints) evolve faster than archival publication cycles, so categorization may lag behind the latest implementations. Third, some sections prioritize breadth over per-paper reproduction details; consequently, reported numbers should be interpreted in the context of each source paper's original setup. Fourth, this survey organizes method comparison primarily through structured tables (the capability matrix, method overview, metric-frequency, and dataset tables). To move beyond tables, Section~\ref{sec:evaluation:gallery} executes a \textit{pilot} of the proposed Golden Protocol as a visual gallery on unified inputs; this pilot is intentionally restricted to three representative methods spanning the major paradigms and is illustrative rather than an exhaustive benchmarking of all $\sim$90 surveyed methods. A full multi-pillar quantitative study across the entire corpus remains future work and a call for community adoption. We therefore view the proposed Golden Protocol as both an evaluation recommendation and a call for standardization in future PST benchmarks.

% (Legacy placeholders removed; content integrated above.)

\appendix
\section{PRISMA Screening Detail}
\label{app:prisma}
Table~\ref{tab:prisma} summarizes our literature screening as a PRISMA-style funnel. The $\sim$480 identified records span the four query categories (portrait stylization, style transfer, GAN inversion, diffusion-based editing) across the listed flagship venues and journals (2015--2025). Of these, $\sim$160 were removed as duplicates (cross-venue reposts and arXiv$\to$conference versions). Title and abstract screening excluded $\sim$140 records for the three stated reasons: purely unconditional generation, non-face stylization, and methods without explicit identity-/structure-preserving controls. Full-text eligibility assessment excluded the remaining candidates that did not meet the identity-/structure-preserving criterion, yielding the final corpus of 115+ publications (88 peer-reviewed venues and 28 screened arXiv preprints).

\section{Surveyed Method Checklist}
\label{app:checklist}
The capability matrix (Table~\ref{tab:comparison_matrix}) samples representative methods across paradigms; the complete surveyed corpus of $\sim$90 distinct PST methods with dedicated stylization components is tracked in the curated BibTeX released by the authors (Section~\ref{sec:discussion:availability}). Approximate paradigm coverage: $\sim$12 NST / optimization \& feed-forward autoencoders, $\sim$25 GAN-based, $\sim$45 diffusion-based, and $\sim$10 autoregressive, plus emerging 3D/NeRF and video stylization works cited as frontier contributions.
