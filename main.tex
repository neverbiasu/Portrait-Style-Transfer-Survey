...
\begin{table*}[t]
\centering
\caption{Capability matrix of representative PST methods. Qualitative symbols: $\bigstar$ = strong/fast; $\blacktriangle$ = moderate; $\bullet$ = weak/slow. Speed figures are reported heterogeneously across the source papers (varying GPUs, resolutions, batch sizes, and backbone sizes) and are therefore \textit{not} directly comparable; they indicate relative orders of magnitude only, under each method's original evaluation protocol.}
